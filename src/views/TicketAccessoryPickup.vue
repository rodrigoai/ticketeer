<template>
  <div class="min-vh-100 bg-light py-4 px-3">
    <div class="container" style="max-width: 640px;">
      ...
      <!-- Already Picked Up -->
      <div v-else-if="ticketData && ticketData.ticket.accessoryCollected">
        <!-- Success Card -->
        <div class="card text-bg-success text-center rounded-4 mb-4">
          <div class="card-body p-5">
            ...
            <h2 class="h5 fw-bold mb-2">KIT RETIRADO</h2>
            <p class="small mb-0">Retirada realizada {{ formatDateTimePortuguese(ticketData.ticket.accessoryCollectedAt) }}</p>
          </div>
        </div>

        <!-- Notes if available -->
        <div v-if="ticketData.ticket.accessoryCollectedNotes" class="mb-4">
          <div class="bg-white rounded-4 shadow-sm p-4">
            <h5 class="fw-bold text-dark mb-2">Observações</h5>
            <p class="text-muted mb-0">{{ ticketData.ticket.accessoryCollectedNotes }}</p>
          </div>
        </div>
      ...
      <!-- Ready for Pickup -->
      <div v-else-if="ticketData && ticketData.canPickup">
        <!-- Header -->
        ...
        <!-- Notes Field -->
        <div class="mb-4">
          <label for="pickup-notes" class="form-label fw-medium text-secondary">Observações (opcional)</label>
          <textarea
            id="pickup-notes"
            v-model="pickupNotes"
      ...
      <div v-else-if="pickupSuccess">
        ...
        <div class="card text-bg-success text-center rounded-4 mb-4">
          <div class="card-body p-5">
            ...
            <h2 class="h5 fw-bold mb-2">KIT RETIRADO</h2>
            <p class="small mb-0">Retirada realizada {{ formatDateTimePortuguese(pickupSuccess.ticket.accessoryCollectedAt) }}</p>
          </div>
        </div>

        <!-- Notes if available -->
        <div v-if="pickupSuccess.ticket.accessoryCollectedNotes" class="mb-4">
          <div class="bg-white rounded-4 shadow-sm p-4">
            <h5 class="fw-bold text-dark mb-2">Observações</h5>
            <p class="text-muted mb-0">{{ pickupSuccess.ticket.accessoryCollectedNotes }}</p>
          </div>
        </div>
        ...
        <div class="mb-4">
          <div class="d-flex align-items-center mb-3">
            ...
            <span class="fw-medium text-secondary">Ticket número #{{ pickupSuccess.ticket.identificationNumber }}</span>
          </div>
          
          <h3 class="h4 fw-bold text-dark mb-1">{{ pickupSuccess.ticket.buyer || 'Nome do Comprador' }}</h3>
          <p class="text-muted mb-1">{{ pickupSuccess.ticket.buyerDocument || '000.000.000-00' }}</p>
          <p class="text-muted mb-1">{{ pickupSuccess.ticket.location || 'Local' }}</p>
          <p class="text-muted mb-0">{{ pickupSuccess.ticket.description }}</p>
        </div>
        ...
        <div>
          ...
          <h3 class="h4 fw-bold text-dark mb-1">{{ pickupSuccess.event.name }}</h3>
          <p class="text-muted mb-1">{{ pickupSuccess.event.venue }}</p>
          <p class="text-muted fw-medium mb-0">{{ formatEventDateTime(pickupSuccess.event.opening_datetime) }}</p>
        </div>
      </div>
    </div>
  </div>
      ...
      try {
        ...
        if (!response.ok) {
          if (data.alreadyCollected) {
            // Update the ticket data to show already picked up
